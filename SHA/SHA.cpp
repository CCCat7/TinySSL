...
unsigned long long SHA512::sha512_k[80] = {
...
void sha512(byte* input, byte *result, size_t len){
    char *inputStr = new char[len * 2 + 1];
    unsigned long ui[len];
    
    for (size_t i = 0; i < len; i++) {
        ui[i] = input[i].to_ulong();
        sprintf(inputStr, "%s%02x", inputStr, (unsigned int)ui[i]);
    }
    inputStr[len * 2] = '\0';
    ...
    SHA512 sha = SHA512();
    sha.init();
    sha.update((const unsigned char*)inputStr, (unsigned int)len);
    sha.final(digest);
